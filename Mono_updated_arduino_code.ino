...
void drawHomePage() {
  ...
  // Title with large bold font on one line
...
void handleHomeTouch(int x, int y) {
  ...
  if (x >= 140 && x <= 340 && y >= 690 && y <= 770) {
    ...
    if (selectedTemp < 0 || selectedVol1 < 0 || selectedVol2 < 0 || selectedConc < 0 || selectedSpeed < 0) {
...
void drawPrintConfirmPage() {
  ...
  // Draw parameter summary with bigger spacing
  int yPos = 130;
  int lineHeight = 85;
