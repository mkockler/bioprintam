...
#define BUTTON_COLOR 0x0010    // Dark blue buttons
#define BUTTON_SELECTED_COLOR 0x4208  // Darker grey for selected
#define TEXT_COLOR 0x0010      // Dark blue text
#define BUTTON_TEXT_COLOR 0xFFFF  // White text inside buttons
...
void drawWelcomePage() {
  ...
  // Title "BioPrint AM" - large and centered
  display.setFont(&FreeSansBold18pt7b);
  display.setCursor(120, 300);
  display.print("BioPrint AM");
  
  // Subtitle "Ready to start printing?"
...
void drawParameterPage(const char* title, int* options, int numOptions, const char* unit) {
  ...
  int boxY = 75;
  int boxWidth = 260;
  int boxHeight = 100;  // Made taller for bigger text
  int buttonSize = 70;
  ...
  display.setFont(&FreeSansBold18pt7b);  // Bigger font for value
  display.setTextColor(BUTTON_TEXT_COLOR);
  ...
  if (tempSelection < 0) {
    ...
    if (unit[0] == '%') {
      ...
      // For temperature, print value then degree symbol then unit
      int charCount = String(tempSelection, 1).length() + 2; // value + degree + C
      int textWidth = charCount * 18;
      int textX = boxX + buttonSize + buttonSpacing + (boxWidth / 2) - (textWidth / 2);
      display.setCursor(textX, boxY + 62);
  ...
  display.setFont(&FreeSansBold12pt7b);
  display.setTextColor(BUTTON_TEXT_COLOR);  // WHITE text
...
void drawParameterSummary(int x, int y, const char* label, float value, const char* unit) {
  ...
  display.setFont(&FreeSansBold12pt7b);  // Made bigger from FreeSans9pt7b
  display.setCursor(x, y + 40);  // Increased spacing
  if (value < 0) {
    display.print("Not Selected");
  } else {
    // Special formatting for concentration
...
void drawPrintPage() {
  display.fillScreen(BG_COLOR); // Use the white background
  
  // "Loading..." text - print as one word to avoid weird spacing
  display.setFont(&FreeSansBold18pt7b);
  display.setTextColor(0xA9CF);
  display.setCursor(130, 370);
